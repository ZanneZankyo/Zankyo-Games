...
float4 FragmentFunction(vertex_to_fragment_t data) : SV_Target0
{
    ...
    color = float4(0.4f, 0.4f, 0.6f, 1.f);

    float3 meshDistance = data.world_position.xyz - MODEL_POSITION;
    ...
    if (len < radius)
    {
        ...
        float fraction3 = fraction * fraction * fraction;
        int index = int(1024 * fraction3);
        color.x += tMusicSpectrum[index] * 3;
        color.z -= tMusicSpectrum[index] * 3;
       
        color.y += 0.8f * fraction2;
    }
